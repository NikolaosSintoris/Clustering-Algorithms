...
public class LVQMain extends JFrame
{
	...
	public LVQMain(String title, int numberOfCentroids)
	{
		super(title);
	    ...
	    // Create chart. 
	    JFreeChart chart = ChartFactory.createScatterPlot("LVQ Scatter Plot", "X-Axis", "Y-Axis", dataset); 
	...
	private XYDataset createDataset(int numberOfCentroids)
	{
		...
		LVQAlgorithm lvqObject = new LVQAlgorithm(numberOfCentroids, learningRate);
		lvqObject.loadClusteringDataSet();
		
		for(int iteration = 1; iteration <= 5; iteration++)
		{
			//System.out.println("ITERATION: " + iteration);
			
			float tempClusteringError = lvqObject.LVQ();

			if(tempClusteringError <= minClusteringError)
			{
				minClusteringError = tempClusteringError;
				bestClusteringDataSetArrayList = lvqObject.getBestClusteringSet();
				bestCentroidsArrayList = lvqObject.getBestCentroids();
			}
		}
	...
	public static void main(String[] args) 
	{
		...
		while(counter <= 6)
		{
		    ...
		    SwingUtilities.invokeLater(() -> {  
		    	LVQMain example = new LVQMain("LVQ Scatter Plot with " + numberOfCentroids + " centroids", numberOfCentroids);  
